<script lang="ts">
  ...
  $: {
    ...
    if (path.includes('/admin/users') || path.includes('/admin/categories') || path.includes('/admin/update')) expandedSections.system = true;
        ...
        <div class="border-b border-primary-50 lg:border-none pb-2 lg:pb-0">
          ...
          {#if expandedSections.system}
            <ul class="mt-1 ml-6 space-y-1 border-l-2 border-primary-200 pl-4 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <li>
                <a href="/dashboard/admin/update" class="flex items-center w-full text-left py-2 px-3 rounded-lg truncate {$page.url.pathname === '/dashboard/admin/update' ? 'bg-primary-500 text-white shadow-sm' : 'hover:bg-primary-50 text-gray-600'} transition-colors duration-200 text-sm font-medium">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
