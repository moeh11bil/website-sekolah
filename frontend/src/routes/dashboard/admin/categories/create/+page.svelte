<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (response.ok) {
        successMessage = data.message || 'Category created successfully!';
        name = '';
        slug = '';
        ...
        errorMessage = data.message || 'Failed to create category.';
      }
    } catch (error: any) {
...
<svelte:head>
  <title>Tambah Kategori Baru - Dashboard Admin</title>
</svelte:head>

<div class="bg-white rounded-xl shadow-lg p-6 max-w-2xl">
  <h2 class="text-2xl font-bold text-primary-800 mb-6">Tambah Kategori Baru</h2>
