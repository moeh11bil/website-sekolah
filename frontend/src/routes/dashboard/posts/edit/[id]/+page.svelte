<script lang="ts">
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (postData) {
          ...
          // Store category_id as received from API, handling the null case for HTML select
          // For HTML select elements, null should be represented as empty string
      ...
      if (!errorMessage) { // Only set error if it's not already set from status check
        errorMessage = e.message || 'Failed to fetch post details.';
      }
    } finally {
      loading = false;
    }
  ...
  async function handleSubmit(newStatus: 'draft' | 'pending_approval' | 'published' = currentStatus) {
    loading = true;
    errorMessage = null;
    successMessage = null;
    ...
    try {
      ...
      // Always send category_id, even if empty string to allow clearing it
      ...
      if (currentUserRole === 'admin') {
      ...
      } else if (currentUserRole === 'teacher' && newStatus === 'published') {
        formData.append('status', newStatus);
      } else if (currentUserRole === 'student' && newStatus === 'pending_approval') {
        formData.append('status', newStatus);
        ...
        if (newStatus !== currentStatus) {
            errorMessage = "Unauthorized status change attempt.";
            loading = false;
            return;
        }
      }
      ...
      if (response.ok) {
        successMessage = data.message || 'Postingan berhasil diperbarui!';
        currentStatus = newStatus; // Update local status
        // Update image URL if a new image was uploaded
        if (data.imageUrl) {
          currentImageUrl = data.imageUrl;  // This will be the relative path like /uploads/posts/filename
        }
      } else {
        errorMessage = data.message || 'Gagal memperbarui postingan.';
      }
    } catch (error: any) {
...
<svelte:head>
  <title>Edit Postingan - Dashboard</title>
  <style>
    #content ul {
      padding-left: 1.5rem;
      margin: 0.5rem 0;
...
<div class="bg-white rounded-xl shadow-lg p-6 max-w-4xl">
  <h2 class="text-2xl font-bold text-primary-800 mb-6">Edit Postingan</h2>
  ...
  <div class="bg-gradient-to-br from-primary-50 to-white p-6 rounded-xl border border-primary-100">
    ...
    {#if loading && !title}
      <div class="flex justify-center items-center h-64">
        <div class="flex flex-col items-center">
          ...
          <p class="text-primary-700">Memuat detail postingan...</p>
        </div>
      </div>
    {:else if !title && !loading}
      <div class="bg-primary-50 border border-primary-100 text-primary-700 px-4 py-8 rounded-lg text-center">
        ...
        <p class="text-lg">Postingan tidak ditemukan.</p>
      </div>
    {:else}
      <form on:submit|preventDefault={() => handleSubmit()}>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div class="flex flex-wrap gap-2">
            <button
              type="submit"
              class="bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all duration-200 transform hover:scale-105 flex items-center"
              disabled={loading}
            >
              {#if loading}
            ...
            {#if currentUserRole === 'admin' || (currentUserRole === 'teacher' && currentStatus !== 'published')}
              <button
                type="button"
                on:click={() => handleSubmit('published')}
                class="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all duration-200 flex items-center"
                disabled={loading}
              >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            ...
            {#if currentUserRole === 'student' && currentStatus === 'draft'}
              <button
                type="button"
                on:click={() => handleSubmit('pending_approval')}
                class="bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all duration-200 flex items-center"
                disabled={loading}
              >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
