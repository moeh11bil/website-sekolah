<script lang="ts">
  ...
  interface GalleryItem {
    ...
    image_url: string;
    category: string;
    status: 'active' | 'inactive'; // Hanya admin yang bisa mengubah status; siswa item secara default 'inactive' hingga disetujui
    created_at: string;
  }
  ...
  let description = '';
  let category = '';
  let status: 'active' | 'inactive' = 'inactive'; // Default inactive untuk siswa
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Siswa tidak boleh mengganti status - selalu inactive untuk siswa
      if (currentUserRole === 'student') {
        formData.append('status', 'inactive');
      } else {
        // Untuk admin atau guru, gunakan status yang dipilih
      ...
      category = '';
      if (currentUserRole === 'student') {
        status = 'inactive'; // Untuk siswa, tetap gunakan 'inactive'
      } else {
        status = 'active';
      }
      selectedImage = null;
  ...
  function startDelete(item: GalleryItem) {
    deletingItem = item;
  }
  ...
  async function confirmDelete() {
    ...
    try {
      const response = await fetch(`${API_URL}/api/admin/gallery/${deletingItem.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      deletingItem = null; // Tutup modal
      await fetchGalleryItems();
    } catch (e: any) {
      error = e.message || 'Terjadi kesalahan saat menghapus data';
      deletingItem = null; // Tutup modal meskipun error
    }
  }
  ...
  function startEdit(item: GalleryItem) {
    ...
    // Jika pengguna adalah siswa, jangan ubah status (tetap inactive)
    if (currentUserRole !== 'student') {
      status = item.status;
    } else {
      // Untuk siswa, status tetap inactive meskipun item sebelumnya aktif
      status = 'inactive';
    }
...
<svelte:head>
  <title>Manajemen Galeri - Dashboard {currentUserRole === 'teacher' ? 'Guru' : 'Siswa'}</title>
</svelte:head>
...
<div class="bg-white rounded-xl shadow-lg p-6 max-w-6xl">
  ...
  {:else}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2">
        <div class="bg-gradient-to-br from-primary-50 to-white p-6 rounded-xl border border-primary-100">
          <h3 class="text-lg font-bold text-primary-800 mb-4">Daftar Item Galeri Saya</h3>
                  ...
                  <div class="p-4">
                    <div class="flex justify-between items-start">
                      ...
                      <span class={`text-xs px-2 py-1 rounded-full ${item.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                        {item.status === 'active' ? 'Aktif' : (currentUserRole === 'student' ? 'Menunggu Konfirmasi Admin' : 'Tidak Aktif')}
                      </span>
                    </div>
                    ...
                    <div class="flex gap-2 mt-3">
                      <button
                        ...
                        disabled={currentUserRole === 'student' && item.status === 'active'}
                      >
                        {currentUserRole === 'student' && item.status === 'active' ? 'Edit (Tidak Bisa)' : 'Edit'}
                      </button>
                      <button
                        ...
                        disabled={currentUserRole === 'student' && item.status === 'active'}
                      >
                        {currentUserRole === 'student' && item.status === 'active' ? 'Hapus (Tidak Bisa)' : 'Hapus'}
                      </button>
                    </div>
                  </div>
                </div>
  ...
  {#if deletingItem}
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full border border-primary-200">
        <h3 class="text-lg font-bold text-red-600 mb-3">Konfirmasi Hapus</h3>
        <p class="text-gray-700 mb-2">Apakah Anda yakin ingin menghapus item galeri ini?</p>
        <p class="text-gray-600 text-sm font-medium mb-4 truncate">{deletingItem.title}</p>
        ...
        <div class="flex justify-end space-x-3">
          ...
          <button
            ...
            class="px-4 py-2 text-white bg-red-500 hover:bg-red-600 rounded-lg transition-colors duration-200"
          >
            Hapus
          </button>
        </div>
